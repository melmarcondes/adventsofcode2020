...
const getManhattanDistance = (input) => {
    ...
    input.forEach(cmd => {
        ...
        if(direction === 'F'){
            number = getNumberByDirection(currentDir, number);
            currentValue = (currentValue + number);
        }else if(direction === 'R' || direction === 'L'){
            if(number !== 180){
                let newValue = parseInt(notCurrentDirValue[1]);
                notCurrentDirValue = [currentDir, currentValue];
                currentValue = newValue;
            }
            let op = (direction === 'R') ? 
                    (a,b) => a + b < 4 ? a + b : a + b - 4 : 
                    (a,b) => a - b > -1 ? a - b : a - b + 4;
            currentDir = getNewDirection(numberOfMov[number], directionsIndexes[currentDir], op);
        }else{
            number = getNumberByDirection(direction, number);
            if(currentDir == direction || isOposite(currentDir, direction)){
                currentValue += number;
            }else{
                notCurrentDirValue[1] = parseInt(notCurrentDirValue[1]) + number;
            }
        }
        //console.log(direction + number + "==>" + currentDir + currentValue +  "==OTHER==>" + notCurrent)
    });
    return Math.abs(currentValue) + Math.abs(notCurrentDirValue[1]);
